...
{
	int prevn = stratum->cur_pooln;
	bool algo_switch = false;
	struct pool_infos *prev = &stratum->pools[stratum->cur_pooln];
	struct pool_infos* p = NULL;

	stratum_free_job(stratum);

	if (pooln < (stratum->num_pools+1)) {
		stratum->cur_pooln = pooln;
		p = &stratum->pools[stratum->cur_pooln];
	} else {
	...
	if (prevn != stratum->cur_pooln) {

		stratum->pool_switch_count++;
		net_diff = 0;
		g_work_time = 0;
		g_work.data[0] = 0;
		stratum->pool_is_switching = true;
		stratum->need_reset = true;
		...
		stratum->pooln = stratum->cur_pooln;

		// unlock the stratum thread
		printf("pushing %s stratum\n", rpc_url);
		tq_push(thr_info[stratum_thr_id].q, strdup(rpc_url));
		applog(LOG_BLUE, "Switch to stratum pool %d: %s", stratum->cur_pooln,
...
{
	int next = 0;
	for (int i=0; i<stratum->num_pools; i++) {
		int pooln = (startfrom + i) % stratum->num_pools;
...
{
	if (stratum->num_pools > 1) {
		int pooln = pool_get_first_valid(infos, stratum->cur_pooln+1);
